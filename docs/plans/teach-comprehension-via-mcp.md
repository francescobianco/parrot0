...
|---|---|---|---|---|
| **U1** | **Variabili esplicite** (sigillo `?X` o marcatura MCP `{"var":...}`; additivo a `is_var`, `parse_term`) | A.1: letterali sicuri; nomi propri insegnabili | basso | `capital(spain, Madrid)` via MCP → `kb.match` torna `Madrid` (oggi `[]`) |
