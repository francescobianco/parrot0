...
| U4 | 2026-09-13 | (vedi git log) | 3/21 | 3/10 | 1 | «how do i / how can i / how should i» chiede un procedimento: la procedura in KB si raggiunge («How do I change a flat tire?», «How can I make tea?» → i passi numerati; prima muro e «no code to show»); senza passi il muro del procedimento, non il saggio di metodo (`analysis_last_resort` legge la propria condotta). `howto` base e held da R a M; `plan.p0t` 13/14 → 14/14. **Non insegnato** il procedimento dell'uovo (fonte: American Egg Board): la chiave del procedimento è solo l'oggetto («egg»), e «How do I fry an egg?» riceverebbe i passi della bollitura — una bugia; serve prima la chiave verbo+oggetto |
| U5 | 2026-09-13 | (vedi git log) | 3/21 | 4/10 | 1 | tristezza: situazione `user_low_mood` e piano insegnato parlando con fonte NHS (dispiacere, medico se dura più di due settimane, domanda); «I feel sad today» da F a U; la preoccupazione italiana riceve il piano insegnato invece di `own_method(worry)` (emotion-it da P a U). [lab](../labs/assistente-utile/2026-09-13-u5.md) |
| U8 | 2026-09-13 | (vedi git log) | 3/21 | 4/10 | 1 | presupposto sbagliato su una relazione: «is rome the capital of france?» da «Paris.» a «Not as far as I know: I know Paris as the capital of France.» (non un «No», che la KB non autorizza: il Sudafrica ha tre capitali e ne tiene una); la coda «…, right?» / «…, vero?» è la polare (`question_tag/1`). Resta muro il `base` «The sun goes around the earth, right?»: manca la lettura di «goes around» come `orbits` e la disgiunzione stella/pianeta. Rossi answerframe 3, blankwall 6+5, fewshot 6, document_claims 63 identici su HEAD |
| U7 | 2026-09-13 | (vedi git log) | 4/21 | 5/10 | 1 | scelta fra due cose: situazione `user_choice` («should i / is it better to / dovrei» + «or / o») e piano insegnato parlando (i criteri, poi «What do you want to do with it first?»; la scelta resta di chi chiede); l'ordinale dentro la scelta non viene più chiarito. `decide` base e held da R/M a U; transfer «Is it better to rent or buy a car?», «dovrei studiare francese o spagnolo?»; contrasto «should i go?» fuori; ablation e ripristino; processo nuovo in italiano. /save: C = 2, P = 6 lacune/provenienza + trascritti, X = 0. Visto e non toccato: «is paris or rome the capital of italy?» → «Yes.» |
| U11a | 2026-09-13 | (vedi git log) | 5/21 | 5/10 | 1 | «What can you help me with?» / «in cosa puoi aiutarmi?»: situazione `user_help_scope` e piano insegnato parlando (le cose con cui si può aiutare, verificate oggi sul banco e nei cricchetti; poi «What do you need right now?»). `capability` base da P a U; «what can you do?» resta al registro delle capacità (domanda su di sé, non servizio). /save: C = 2, X = 0. Visto: «what is the boiling point of water?» → la definizione dell'acqua (misclaim, non promesso nell'elenco) |
| U6a | 2026-09-13 | (vedi git log) | 6/21 | 5/10 | **0** | confronto fra due cose che la KB definisce: le due definizioni una accanto all'altra («Here is what I know of each. A virus is… A bacterium is…») al posto del metodo generico. **Ultimo F del banco `base` tolto.** Restano al saggio di metodo le coppie senza definizioni («a cat and a dog», «a lake and a sea»): il saggio resta aiuto finto per quelle, da affrontare con i fatti dei due termini |
| U6b | 2026-09-13 | (vedi git log) | 7/21 | 6/10 | 0 | la cortesia davanti a una richiesta: `request_wrapper/2` («can you explain X?» → «explain X»), dopo gli involucri delle domande; «Can you explain what inflation is?» e «…what a vaccine does?» da R («I don't know whether you can explain…») a U. Italiano non dichiarato: «puoi spiegarmi cos'è l'inflazione?» non arriva alla lettura, da guardare con la canonicalizzazione |
| U12a | 2026-09-13 | (vedi git log) | 8/21 | 6/10 | 0 | il conto quotidiano «X a month, how much in two years?»: tre pezzi del turno (quantità con unità, cadenza, durata) e la conversione della durata con `quantity/3`, tutto KB (`numeric-questions.p0` §4). «1200 euros.», «42 euros.» per 3 euro al giorno in 2 settimane. `math` base da M a U. Trovato: un predicato oltre `KB_MAX_ARGS` (4) o una regola oltre 16 goal vengono scartati; il caricatore lo dice (`kb_load: PARSE ERROR`), ma la sonda via MCP scartava stderr — ora `useful-probe.py` controlla il boot prima di sondare. Rossi di `tests/p0t/math` identici su HEAD |
| U12b | 2026-09-13 | (vedi git log) | 9/21 | 7/10 | 0 | aiuto a scrivere un messaggio: situazione `user_writing` e piano insegnato parlando (la forma di un messaggio chiaro, con fonte National Archives / Style Manual australiano; poi «Who is it for, and what do you want it to get done?»). «Help me write an email.» da R («no code to show») a U, «Can you help me with a letter?» da R a U; il codice e la poesia restano ai loro lettori; processo nuovo in italiano. /save: C = 2, X = 0 |
| U13 | 2026-09-13 | (vedi git log) | 10/21 | 8/10 | 0 | cominciare qualcosa di nuovo: situazione `user_getting_started` («where do I begin / where should I start / da dove comincio») e piano insegnato parlando con fonte NHS Couch to 5K (cominciare in piccolo e con regolarità; «How much time can you give it each week?»). `plan` base e held da M a U; transfer chitarra, spagnolo in italiano; contrasto «where does the river begin?» fuori. /save: C = 2; X = 1 in quarantena (`pending_gap_failed(running)`, artefatto della ricerca senza rete dopo l'ablation) |
| U14 | 2026-09-13 | (vedi git log) | 11/21 | 8/10 | 0 | organizzare la giornata: situazione `user_organizing_time` e piano insegnato parlando con fonte NHS (scrivere, scegliere le due o tre cose che contano, farle per prime; «Quale parte della giornata ti è più difficile da gestire adesso?»). `italian` base da M a U. **Tre strade interrotte curate:** il pezzo `named` non inizializzava `is_text` (una vista della forma ritirava «"user organizing time"» e non trovava niente: l'ablation cadeva nell'analisi); il primo passaggio dell'analisi non leggeva la condotta dichiarata; le lezioni di piano non erano dichiarate come tali (`intent_cue(plan_lesson, …)`). Trappole di superficie annotate: «first», «what matters most» (own_method), mosse oltre otto parole. Contrasto debole: «the company wants to organize my day» riceve il piano |
| U15 | 2026-09-13 | (vedi git log) | 12/21 | 8/10 | 0 | idee regalo: situazione `user_gift_ideas` (con chi parla: «the present for the class» resta fuori) e piano insegnato parlando (qualche genere di regalo — hobby, esperienza da condividere, qualcosa di personale — e «What does the person enjoy doing in their free time?»). `brainstorm` base da M a U. **Strada interrotta curata:** una lezione di piano che nomina la situazione («when someone needs gift ideas then …») veniva servita dal piano stesso invece che letta come lezione (`turn_is_plan_lesson`). /save: C = 2, X = 1 in quarantena (`pending_gap_failed(present)`) |
| U16 | 2026-09-13 | (vedi git log) | 13/21 | 9/10 | 0 | una mossa può dichiarare la propria condizione (`move_condition(safety_first, hazard_cue)`): la sicurezza si dice per l'auto, i freni, la lavatrice, la caldaia, non per il wifi o lo schermo del telefono. `troubleshoot` base da P a U, held `car` resta U con la sicurezza |
| U17 | 2026-09-13 | (vedi git log) | 14/21 | 9/10 | 0 | cucinare per ospiti e il vincolo del turno dopo: `user_hosting_meal` (menu semplice; «Does anyone have dietary needs, and how much time do you have?») e `user_guest_diet` (vincolo alimentare **e** riferimento al gruppo: «one of them», «uno di loro»; un piatto che vada bene per tutti). `multi-turn` da M+M a U+U; «vegetarians don't eat meat» resta un'affermazione. Quarantena: `not(ate(vegetarians, meat))` — «don't eat» letto come il passato «ate», lettura sbagliata da curare. La continuità è per indizi del turno, non ancora per memoria del turno prima |
| U18 | 2026-09-13 | (vedi git log) | 14/21 | 9/10 | 0 | la risposta sullo scopo è una frase: «What is a compass used for?» da «"finding north without landmarks".» a «A compass is used for finding north without landmarks.» (`turn_form_reply` per le forme `answer_relation`; l'articolo solo se chi chiede l'ha usato). `followup-offer` resta P: manca ancora il passo successivo fondato (T6) |
| U19 | 2026-09-13 | (vedi git log) | 14/21 | 9/10 | 0 | il caso di F. «ho un problema elettrico» → «ci sono scintille»: i segni di pericolo sono conoscenza di situazione (K11: scintille, odore di gas, piani insegnati con fonte) e la risposta alla domanda di parrot0 si legge sul **tabellone unico** (`issues.p0`, genere `inquiry`, dialogica L1–L2): risposta → seguito della situazione, chiusura, riapertura. Il seguito di un seguito non è ancora tracciato; `/save` non persiste i ritiri. [lab](../labs/assistente-utile/2026-09-13-u19.md) |
