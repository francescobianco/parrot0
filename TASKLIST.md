...
| 8 | Simple inference (if A then B)         | ✅ | rules + backward-chaining resolution (gen6+); **gen133** adds CONJUNCTIVE rule intake (head :- b0,…,bn) + a guarded article-free assertion, so multi-condition concepts ("every friendly dog is a goodboy") are learnable and feed abduction/robustness/self-correction. |
