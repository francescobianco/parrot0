...
TEACHER_SYS = (
    ...
    "- GRAMMATICAL-CONSTRUCT TOOLKIT (KB-first morphology). A sentence that fails on "
    "a GRAMMAR construct (conjugation, an auxiliary, negation, elision, subject-drop) "
    "is still compositional: teach the construct's FACT and the engine composes it. "
    "NEVER memorize the whole sentence as a tr_*_phrase, and NEVER invent a predicate "
    "not in this list (unlisted preds are dropped and teach nothing). The engine "
    "tracks the subject and applies these facts in order; pick the ones the sentence "
    "needs:\n"
    "  * conj_es(english_verb, subject, spanish_form) / conj_fr(...): person-indexed "
    "verb form. subject is the english pronoun (i, you, he, we, they). e.g. "
    "conj_es(need, i, necesito), conj_es(speak, you, hablas), conj_fr(learn, i, "
    "apprends). The presence of a conj fact makes its subject word recognized.\n"
    "  * pro_drop(es): declare the language drops the subject pronoun (Spanish/Italian). "
    "So 'I need help' -> 'necesito ayuda' (I dropped). Teach once per language.\n"
    "  * aux_progressive(am|is|are): drop the progressive auxiliary before an -ing verb "
    "('I am learning' -> the -ing verb glosses finite via conj). Teach the missing aux.\n"
    "  * aux_question(do|does): drop the question auxiliary ('do you speak English' -> "
    "'¿hablas inglés?', the verb conjugates). Teach the missing aux.\n"
    "  * negation_es(\"don't\", no): a negation marker glossing to 'no' ('I don't "
