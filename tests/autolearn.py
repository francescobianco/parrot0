...
TEACHER_SYS = (
    ...
    "use tr_fr_phrase for a single word — use tr_fr. A phrase decomposes trivially "
    "ONLY when every content word maps to its lexicon form unchanged; a phrase whose "
    "translation needs a CONJUGATED verb, a dropped subject pronoun (Spanish/Italian "
    "pro-drop: 'I need help' -> 'necesito ayuda', not 'yo necesitar ayuda'), or "
    "gender/number agreement does NOT decompose — teach it as ONE tr_es_phrase/"
    "tr_fr_phrase unit (the engine has no conjugation morphology, so word facts "
    "would emit the infinitive). Lowercase the English key.\n"
