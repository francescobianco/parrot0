...
| `read: <frase con un verbo di relazione>` | **la IR legge la frase e la KB la impegna** (specie A), anche con nomi mai visti e sintagmi con articolo: «read: Beetles threaten forests.», «read: The zorvan beetle threatens the kelda forest.» → `semantic_binding(binary(threaten), …)`. La domanda sull'oggetto («what do beetles threaten?») risponde **dalla lettura** (porta `turn_response`), e `who answered?` non dice `answerframe` (19 settembre 2026, `ir_reading_answer.p0t`). Vale anche per la frase detta nel turno senza `read:`, e anche per la domanda sul **soggetto** («what/who threatens forests?»). Dal 19 settembre pomeriggio la lettura regge anche: il **passivo** («X is threatened by Y»), la **participiale** («…, displacing Y» ha il soggetto della principale), l'**elenco** di oggetti («supplies A, B and C» → la domanda risponde con l'elenco) e il **complemento preposizionale** («reduces dependency on X» → «what does compost reduce dependency on?» → X). Dalla sera del 19 settembre anche la **relazione aggettivale** già insegnabile («"rich in" is an adjective relation»): «The resulting mixture is rich in A and B» → `semantic_binding(binary(rich_in), …)` con l'elenco, e «what is the resulting mixture rich in?» risponde dalla lettura. ⚠ La IR tace, e allora risponde il percorso di prima, quando l'oggetto è una sequenza di più parole **senza articolo** («hard carbonate exoskeletons»), quando fra verbo ed entità c'è una parola piena non letta, e quando la domanda porta il gap dopo una preposizione («in what water …») |
...
| `forget that X Y is a relation` (e ogni `forget that <soggetto di più parole> is a <classe>`) | **RI-018** — il ritiro prende come soggetto tutto ciò che precede la copula: «forget that operating voltage is a relation» toglie `relation(operating_voltage)`. E la domanda sul valore («What is the operating voltage of a USB port?») è servita dalla lettura quando il lettore dei frame la sa leggere e la KB ha il valore (`turn_frame_answer/2`): prima il frasario rispondeva con la definizione del sistema operativo |
...
| `P is an adverbial particle` (e `forget that P is an adverbial particle`) | **RI-016** — la particella fa parte del **significato** del verbo: con `P` avverbiale il predicato unisce radice e particella (`step_up`, `step_down`, `break_down`) nella lettura, nella IR e nella domanda, invece della sola radice (che resta per le particelle che introducono l'oggetto: `flow into` → `flow`). «A flyback transformer steps up the voltage.» → `step_up(flyback_transformer, voltage)`; «What steps down the voltage?» non confonde più i due verbi. Una particella avverbiale è anche una **particella di verbo**, insegnabile prima del suo verbo. Ritiro provato: senza la lezione torna la lettura per radice |
| `"X" is a clitic` (e `forget that "X" is a clitic`) | **§29 L3** — `word_clitic/1`: un pezzo che, **in coda a una parola**, se ne stacca e diventa una parola a sé nel turno canonico, dopo le contrazioni («what's», «it's» si sciolgono prima). `word_clitic` parte vuoto. Dopo `"'s" is a clitic`, la lezione di forma `what is x's y means what is the y of x` si impara (prima era rifiutata: «x's» era una parola sola e la variabile x non c'era), e vale per **ogni** nome di relazione: «What is France's capital?» → Paris, «What is ethanol's boiling point?» → 78 °C, «What is Hamlet's author?» → shakespeare. Ritiro provato: il genitivo torna muro. ⚠ «Who is Hamlet's author?» vuole la stessa forma con «who»; l'affermazione «France's capital is Paris.» scrive `france s capital`, entità spazzatura (banco `docs/labs/l3/clitico/genitivo.p0t`) |
| `"C" is a word joiner` (e `forget that "C" is a word joiner`) | **RI-017** — `word_joiner/1`: un carattere che, **fra due lettere**, tiene insieme una parola per i tokenizzatori del turno e della IR. Dopo `"-" is a word joiner`, «a step-down transformer», «a heat-resistant glove», «a well-known protocol» arrivano alla IR come un sintagma intero invece di perdere il modificatore («What protects the hand?» → *Heat-resistant glove*, prima *Resistant glove*). Fra due cifre il trattino resta un confine («30-375» sono due numeri). Ritiro provato: la IR torna a spezzare |
| `the <nome> of x is y means x <verbo> y` (e `forget that …`, articolo compreso) | **RI-020** — un **nome di relazione** si lega a una relazione che la KB **tiene già**, nominata dalle sue parole (`relation_named_by/2`: «boils at» → `boils_at`): «the boiling point of x is y means x boils at y» fa rispondere «What is the boiling point of water?» / «What's the boiling point of ethanol?» dai fatti `boils_at` esistenti, e la frase «The boiling point of acetone is 56 degrees Celsius.» scrive `boils_at(acetone, 56_degrees_celsius)`. La domanda si legge sul verso copulare inverso («@O is the boiling point of @S»), dichiarato in KB per ogni costruzione che finisce con copula + valore. Ritiro provato: la domanda torna al frasario. ⚠ la forma direttiva «Tell me the boiling point of X» non passa ancora; senza lezione «what is the R of X» con R ignoto riceve ancora la definizione di X |
...
| `"X" is a contraction of "Y"` (e `forget that "X" is a contraction of "Y"`) | **RI-022** — una **contrazione**: `phrase_canon(X, Y)`, la stessa classe di «what's» → «what is», quindi vale per ogni lettore a valle. `"i'd like" is a contraction of "i would like"` fa rispondere «I'd like to know the melting point of tin.» (232 °C) invece di scrivere un fatto falso. Le riscritture vanno al punto fisso (fino a tre passi): la contrazione arriva alla parafrasi di RI-021. Insegnare la contrazione **con il contesto** che la rende non ambigua («'d» è *would* in «i'd like», *had* in «i'd seen»). Ritiro provato. ⚠ l'iniziativa di dialogo legge i token grezzi: «I'd like to learn X» non apre ancora l'attività |
| `X is short for Y` · `forget that X is short for Y` (sigle, dal vivo) | **RI-023** — la sigla ora **entra, si salva e si ritira**: `LED is short for light-emitting diode` → «What does LED stand for?», «What does an LED emit?» dopo un fatto detto sul nome esteso; `DC is short for direct current` → «Do capacitors block DC?» → Yes da un fatto già in KB. Una sigla che è anche una forma di parola («led», passato di *lead*) si riconosce **solo in maiuscolo** (`alias_needs_capitals/1`): «The guide led the climbers» non viene riscritto. Prima di RI-023 la lezione cadeva nel «quasi una lezione» e, quando entrava, finiva nello strato riflessivo e non si salvava |
...
| `the expression X marks Y` · `forget that the expression X marks Y` | `expression_reading/2` — una lettura grammaticale o pragmatica della superficie X, con il nome pronunciato della lettura (`reading_name/2`); vale subito anche sui documenti gia' trattenuti, e si puo' **richiedere**: «what does X mark?» (19 settembre 2026, `english_grammar_growth.p0t`) |
| `X is an english verb lemma` · `forget that …` | `verb_lemma/1` — da li' la morfologia tipata produce terza persona, passato, participio e gerundio (`verb_form/3`), senza una riga per forma. ⚠ misurato: la lezione e il suo ritiro ricostruiscono `extract_frame` (2,7 s) |
| `X is a mass noun` · `X is a transitive verb` · `X is a verb of constitution` | le **facce pronunciabili** delle distinzioni della fogliata (`kb/core/english-grammar/naming.p0`): classi unarie derivate per regola da `noun_feature/3`, `verb_valency/2`, `verb_family/2`. Si chiedono nello stesso modo («is water a mass noun?»). ⚠ la regola generale: una distinzione tenuta SOLO in una relazione a 2-3 argomenti non e' nominabile, quindi per la meta-lingua non esiste |
...
| `the italian for X is Y` applicata a un verbo o a una lingua | le domande su di sé la leggono subito: «sai parlare italiano?», «puoi parlare inglese?», «che lingue parli?» passano per la glossa (`tr/2`) e rispondono dalla conoscenza di sé (`kb/core/self-ability.p0`, 19 settembre 2026). Quali lingue parrot0 parla **non si insegna** con una frase: è `language_name/2` (le lingue di cui ha nome e marcatori), e una lingua nuova parlata è una lingua che parrot0 sa riconoscere e in cui sa rispondere. ⚠ Le parole-lingua (`language_word/2`) e le etichette (`language_label/3`) sono ancora fatti, non lezioni. La stessa lettura vale per **ogni abilità**: «can you read?», «sai nuotare?» rispondono da `self_can/1` (abilità legate a una facoltà reale) e da `bodily_action/1` (no: serve un corpo); l'infinito italiano si riconosce dalla glossa della terza persona («the italian for swim is nuota» basta per «sai nuotare?»). ⚠ Che cosa parrot0 sa fare non si insegna ancora parlando: «you can X» non scrive `self_can/1` |
