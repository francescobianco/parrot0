                    ...
                    if (has_called) {
                      ...
                      if (!kb_response_slots(b, "possession_named_ack", s, 2, msg, sizeof msg))
                        ...
                        { const KbResponseSlot _rs[] = { { "thing", thing }, { "n", n } };
                          if (!kb_response_slots(b, "got_it_your_x_is_x", _rs, 2, msg, sizeof msg))
                ...
                if (isx < nw && isx > mi + 2 && isx + 1 < nw) {
                    ...
                    if (parse_value(val, &dv)) {
                        ...
                        { const KbResponseSlot _rs[] = { { "disp", disp }, { "val", val } };
                          if (!kb_response_slots(b, "got_it_your_x_is_x_2", _rs, 2, msg, sizeof msg))
    ...
    /* gen148 (E4): ordinary user-model facts beyond name/possessions. */
    {
        ...
        if (val_from) {
            ...
            { const KbResponseSlot _rs[] = { { "verb", verb }, { "val", val } };
              if (!kb_response_slots(b, "got_it_you_x_x", _rs, 2, msg, sizeof msg))
...
static void canonicalize_lang(Brain *b, const char *norm, char *out, size_t out_size) {
    ...
    for (size_t i = 0; i < nw && off + 1 < out_size; i++) {
        ...
        if (lex_class_member(b, "10_memory_knowledge_lex1412", tok) && i + 1 < nw && strcmp(w[i + 1], "nome") == 0) {
...
static int join_entity_span(Brain *b, char **w, size_t start, size_t end,
    ...
    for (size_t i = start; i < end; i++) {
        ...
        if (lex_class_member(b, "10_memory_knowledge_lex2850", t) && i + 1 < end && !strcmp(strip_edge_punct(w[i + 1]), "s")) {
            t = (char *)"usa"; i++;
        } else if (lex_class_member(b, "10_memory_knowledge_lex2852", t) && i + 1 < end && !strcmp(strip_edge_punct(w[i + 1]), "is")) {
            t = (char *)"it_is"; i++;
...
static int extract_class_statement(Brain *b, const char *norm,
   ...
   if (!kb_response_slots(b, "learned_created_by_x_x_x", _rs, 3, msg, sizeof msg))
     snprintf(msg, sizeof msg, "Learned: created_by(%s, %s, %s).", subj2, obj2, canon);
   ...
   if (!kb_response_slots(b, "learned_located_in_x_x", _rs, 2, msg, sizeof msg))
     snprintf(msg, sizeof msg, "Learned: located_in(%s, %s).", subj, obj);
   put(msg, out, out_size); } return 1;
            }
        }
        return 0;
    }
    if (lex_class_member(b, "10_memory_knowledge_lex4586", w[p]) && p + 1 < n && !strcmp(w[p + 1], "of")) {
        size_t os = p + 2; if (os < n && p0_lead_det(b, w[os])) os++;
        if (os < n && p0_join(w, os, n, obj, sizeof obj)) {
            kb_set_origin(b->kb, KB_SESSION);
            const char *la[] = { subj, obj };
            if (kb_assert(b->kb, "part_of", la, 2)) {
                ...
                char msg[256]; { const KbResponseSlot _rs[] = { { "subj", subj }, { "obj", obj } };
   if (!kb_response_slots(b, "learned_part_of_x_x", _rs, 2, msg, sizeof msg))
     snprintf(msg, sizeof msg, "Learned: part_of(%s, %s).", subj, obj);
   put(msg, out, out_size); } return 1;
            }
        }
        return 0;
            ...
            for (size_t i = 0; i < nsl; i++) {
                ...
                if (!kb_response_slots(b, "forgotten_ack", rs, 1, msg, sizeof msg))
                    { const KbResponseSlot _rs[] = { { "slotname", slotname } };
                      if (!kb_response_slots(b, "done_i_ve_let_go_of_your_x", _rs, 1, msg, sizeof msg))
...
static int mod_knowledge(Brain *b, const char *norm, const char *raw,
            ...
            if (kb_query(b->kb, "proposition_seen", sq, 1)) {
                ...
                if (ok) {
                    ...
                    { const KbResponseSlot _rs[] = { { "slug", slug } };
                      if (!kb_response_slots(b, "learned_holds_x", _rs, 1, msg, sizeof msg))
    ...
    if (kb_cue_match(b, "10_memory_knowledge_cue9019", norm) && strstr(norm, "'s ")) {
        ...
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex9047", w[i]) && !strcmp(w[i + 1], "is") &&
    ...
    if (kb_cue_match(b, "10_memory_knowledge_cue9095", norm) && kb_cue_match(b, "10_memory_knowledge_cue9095_2", norm) &&
        ...
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex9102", w[i]) && !strcmp(w[i + 2], "are")) {
    ...
    if ((kb_cue_match(b, "10_memory_knowledge_chain9170", norm)) &&
        ...
        char P[64] = "", Q[64] = "";
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex9163", w[i]) && !strcmp(w[i + 2], "are")) {
                size_t j = i + 3;
        ...
        for (size_t i = 0; i + 3 < n; i++) {
            if (lex_class_member(b, "10_memory_knowledge_lex9173", w[i]) && !strcmp(w[i + 2], "are") &&
                strcmp(w[i + 3], "not") && strcmp(w[i + 3], "definitely")) {
                if (!sA[0]) { KSING(sA, w[i + 1]); KSING(sB, w[i + 3]); }
            }
            if (lex_class_member(b, "10_memory_knowledge_lex9177", w[i]) && !strcmp(w[i + 2], "are")) {
    ...
    if (kb_cue_match(b, "definite_conclusion_query", norm) &&
        ...
        char A[64] = "", B[64] = "", C[64] = "";
        for (size_t i = 0; i + 3 < n; i++) {
            if (!A[0] && lex_class_member(b, "10_memory_knowledge_lex9212", w[i]) && !strcmp(w[i + 2], "are")) {
                snprintf(A, sizeof A, "%s", w[i + 1]);
                snprintf(B, sizeof B, "%s", w[i + 3]);
            }
            if (!C[0] && lex_class_member(b, "10_memory_knowledge_lex9216", w[i]) && !strcmp(w[i + 2], "are")) {
    ...
    if ((kb_cue_match(b, "definite_conclusion_query", norm) ||
        ...
        char A[64] = "", B[64] = "";
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex9255", w[i]) && !strcmp(w[i + 2], "are")) {
                snprintf(A, sizeof A, "%s", w[i + 1]);
                snprintf(B, sizeof B, "%s", w[i + 3]);
                break;
            }
     ...
     * universal AND subject of the second) and the question re-names A. */
    {
        ...
        if (nall >= 2 && ynq && !kb_cue_match(b, "10_memory_knowledge_lex9297", norm) && !strstr(norm, " no ")) {
            ...
            char A[64] = "", B[64] = "";
            for (size_t i = 0; i + 3 < n; i++)
                if (lex_class_member(b, "10_memory_knowledge_lex9303", w[i]) && !strcmp(w[i + 2], "are")) {
                    snprintf(A, sizeof A, "%s", w[i + 1]);
    ...
    if (kb_cue_match(b, "10_memory_knowledge_lex9332", norm) &&kb_cue_match(b, "10_memory_knowledge_lex9332_2", norm) && strstr(norm, "?")) {
        ...
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex9338", w[i]) && !strcmp(w[i + 2], "are")) {
    ...
    if ((kb_cue_match(b, "10_memory_knowledge_chain9396", norm)) &&
        ...
        for (size_t i = 0; i + 3 < n; i++) {
            if (lex_class_member(b, "10_memory_knowledge_lex9385", w[i]) && !strcmp(w[i + 2], "are")) {
                snprintf(A, sizeof A, "%s", w[i + 1]);
                snprintf(B, sizeof B, "%s", w[i + 3]);
                break;
            }
    ...
    if ((kb_cue_match(b, "10_memory_knowledge_chain9441", norm)) && kb_cue_match(b, "10_memory_knowledge_cue9423", norm) &&
        ...
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex9430", w[i]) && !strcmp(w[i + 2], "are")) {
...
#define P0_SING(dst, src) do { snprintf(dst, sizeof dst, "%s", src); \
        ...
        char A[64] = "", B[64] = "";
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex9657", w[i]) && !strcmp(w[i + 2], "are")) {
...
#undef P0_SING
    ...
    if (!kb_cue_match(b, "10_memory_knowledge_lex10242", norm) && !kb_cue_match(b, "10_memory_knowledge_lex10242_2", norm) &&
        ...
        for (size_t i = 0; i + 3 < n; i++)
            if (lex_class_member(b, "10_memory_knowledge_lex10250", w[i]) && (!strcmp(w[i + 2], "a") || !strcmp(w[i + 2], "an")) &&
    ...
    if ((strstr(norm, " is like ") || strstr(norm, " are like ")) &&kb_cue_match(b, "10_memory_knowledge_lex10587", norm) &&kb_cue_match(b, "10_memory_knowledge_lex10588", norm)) {
        ...
        for (size_t i = 0; i < nn; i++) {
            ...
            if (lex_class_member(b, "10_memory_knowledge_lex10602", ww[i]) && i >= 1 && !strcmp(ww[i - 1], "happens")) {
                size_t f = i + 1;
   ...
   if (!kb_response_slots(b, "it_boils_at_x", _rs, 1, msg, sizeof msg))
     snprintf(msg, sizeof msg, "It boils at %s.", p);
   put(msg, out, out_size); }
            return 1;
        }
    }
        ...
        if (want_container) {
            ...
            if (x && kb_concept_mentioning(b->kb, x, ckey, sizeof ckey,
                ...
                { const KbResponseSlot _rs[] = { { "x", x }, { "ckey", ckey }, { "cdesc", cdesc } };
                  if (!kb_response_slots(b, "x_is_part_of_x_x", _rs, 3, msg, sizeof msg))
