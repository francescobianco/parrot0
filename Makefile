...
test: test-engine
	...
	@./$(BIN) --test tests/p0t/knowledge/deep_memory.p0t
	...
	@./$(BIN) --test tests/p0t/conversation/dialogue_board.p0t
	@./$(BIN) --test tests/p0t/conversation/retention.p0t
	...
	@./$(BIN) --test tests/p0t/code/code_obligation_plan.p0t
	...
	@./$(BIN) --test tests/p0t/language/contextual_denotation.p0t
	@./$(BIN) --test tests/p0t/meta/three_axis_gap.p0t
...
# ── bench-coverage — CHE COSA I BANCHI NON METTONO MAI ALLA PROVA (gen435) ──
#
# ⛔ NON e' un audit della KB, e il nome precedente (`make audit`) invitava
# esattamente la lettura sbagliata: cercare le lacune con una scansione a freddo
# e colmarle. Quello e' un DEFRAG della KB, ed e' fuori dal perimetro del piano
# (docs/plans/autocrescita.md §0a): una lacuna non e' un'assenza nella KB, e' un
# ARRESTO nell'inferenza, e si scopre DENTRO un turno.
#
# Cio' che questo comando misura e' un'altra cosa, ed e' onesta: quali meccaniche
# dichiarate questi corpora non esercitano mai — cioe' dove le PROVE sono strette.
bench-coverage:
	@$(MAKE) --no-print-directory build >/dev/null
	@PARROT0_LANG=en ./$(BIN) --coverage tests/hundred/hundred.qa --coverage tests/measure/
